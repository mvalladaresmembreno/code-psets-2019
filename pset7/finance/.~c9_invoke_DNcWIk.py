...
def index():
    ...
    transactions = db.execute("SELECT symbol,SUM(share) as shares, price_per_share as price,SUM(TOTAL) as total  FROM transactions  WHERE user_id = :id GROUP BY symbol, price_per_share", id=session["user_id"])
...
def sell():
    ...
    if request.method == "POST":
        ...
        # TODO = transaccion
        db.execute("UPDATE users SET cash = cash + :price_per_share WHERE id = :user_id", price_per_share=total_price, user_id=session["user_id"])
        db.execute("INSERT INTO  transactions (user_id, symbol, share, price_per_share, TOTAL) VALUES (:user_id, :symbol, :share, :price, :TOTAL)",
        user_id=session["user_id"],
        symbol=request.form.get("symbol"),
        share=share*-1,
